...
from app.schemas.customer import CustomerAdminListResponse, CustomerAdminDetailResponse
...
@router.patch("/{customer_id}/status", response_model=BaseResponse[CustomerAdminDetailResponse])
async def update_customer_status(
    ...
    repo = CustomerRepository(db)
    customer = await repo.update_status(customer_id, status_update.status)
    if not customer:
        raise HTTPException(status_code=404, detail="Customer not found")
