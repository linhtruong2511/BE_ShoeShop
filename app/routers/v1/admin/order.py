...
async def get_all_orders(
    ...
    stmt = select(Order).options(selectinload(Order.details)).where(where_clause).order_by(Order.created_at.desc()).offset(skip).limit(limit)
...
async def get_order_detail(order_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_admin)):
...
async def update_order_status(order_id: int, payload: OrderStatusUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_admin)):
    repo = OrderRepository(db)
    order = await repo.get_by_id(order_id)
    if not order: raise HTTPException(404, "Order not found")
    
    order.order_status = payload.status
    log = OrderStatusLog(order_id=order_id, new_status=payload.status, note=payload.note)
    db.add(log)
    await db.commit()
    return BaseResponse(data=None)

@router.patch("/{order_id}/cancel", response_model=BaseResponse)
async def admin_cancel_order(order_id: int, payload: OrderCancel, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_admin)):
    repo = OrderRepository(db)
    order = await repo.get_by_id(order_id)
    if not order: raise HTTPException(404, "Order not found")
    
    order.order_status = "cancelled"
    log = OrderStatusLog(order_id=order_id, new_status="cancelled", note=f"Admin cancelled: {payload.reason}")
    db.add(log)
    await db.commit()
    return BaseResponse(data=None)
