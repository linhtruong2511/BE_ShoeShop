...
class Order(Base, TimestampMixin):
    ...
    customer_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("Customer.customer_id"), nullable=True)
    ...
    payment_method: Mapped[str] = mapped_column(Enum("cod", "bank_transfer", "online"), nullable=False)
    payment_status: Mapped[str] = mapped_column(Enum("pending", "paid", "refunded"), nullable=False, server_default="pending")
    voucher_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("Voucher.voucher_id"), nullable=True)
    voucher_code_snapshot: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    subtotal_amount: Mapped[float] = mapped_column(Numeric(15, 0), nullable=False)
    voucher_discount_amount: Mapped[float] = mapped_column(Numeric(15, 0), nullable=False, server_default="0")
    shipping_fee: Mapped[float] = mapped_column(Numeric(15, 0), nullable=False, server_default="0")
    ...
    status_logs: Mapped[list["OrderStatusLog"]] = relationship("OrderStatusLog", back_populates="order", order_by="OrderStatusLog.changed_at")
