...
class Cart(Base, TimestampMixin):
    ...
    customer_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("Customer.customer_id"), nullable=True, index=True)
    user_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("User.user_id"), nullable=True, index=True)
    session_id: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, index=True)
    status: Mapped[str] = mapped_column(Enum("active", "checked_out", "abandoned"), nullable=False, server_default="active")
