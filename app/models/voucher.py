...
class Voucher(Base, TimestampMixin):
    ...
    voucher_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    ...
    discount_type: Mapped[str] = mapped_column(Enum("percent", "fixed", "amount"), nullable=False)
    discount_value: Mapped[float] = mapped_column(Numeric(15, 2), nullable=False)
    min_order_amount: Mapped[float] = mapped_column(Numeric(15, 0), nullable=False, server_default="0")
    ...
    status: Mapped[str] = mapped_column(Enum("active", "hidden", "expired"), nullable=False, server_default="active")
